...
public class TimeLineDecorator extends ReportDecorator {
    ...
    public TimeLineDecorator(ReportComponent wrapped, MissionEntity mission) {
        super(wrapped);
    ...
    public String build() {
        ...
        List<OperationEventEntity> timeline = mission.getOperationTimeline();
        if (timeline != null && !timeline.isEmpty()) {
            sb.append("Таймлайн происходивших событий:").append("\n");
            for (int i = 0; i < timeline.size(); i++) {
                OperationEventEntity event = timeline.get(i);
